...
var hanabis = [];
var hinokos = [];
var colors = ['#FF0000', '#00FFFF', '#008000', '#FFFF00'];
var nightSkyHeight = 300;
...
var target = document.getElementById('nightSky');
var offsetX = void 0;
var offsetY = void 0;
...
function createHanabi() {
  var x = offsetX;
  var deadY = offsetY; //deadYは爆発する位置のy座標
  var deadWidth = 120 + Math.floor(Math.random() * 50);
  var widePoint = 10 + Math.floor(Math.random() * 2) * 2;
  var moveY = 5 + Math.floor(Math.random() * 3) * 4;
  var y = nightSkyHeight;
  var width = 2;
  var height = 15;
...
function createHinokos(x, y, color) {
  var width = 4;
  var height = 4;
  var positionMax = 3 + Math.floor(Math.random() * 2);
  for (var position = 0; position < positionMax; position++) {
    for (var i = 1; i <= hinokoMany; i++) {
      var direction = 360 * i / hinokoMany;
      var createX = x + Math.cos(getRadian(direction)) * 12 * position;
      var createY = y + Math.sin(getRadian(direction)) * 12 * position;
      var hinoko = document.createElement("div");
...
function moveHanabis() {
  var _loop = function _loop(_index) {
    var hanabiObj = hanabis[_index];

    if (hanabiObj.y <= hanabiObj.deadY) {
      hanabis.splice(_index, 1);
      _index -= 1;
  ...
  for (var index = 0; index < hanabis.length; index++) {
...
function moveHinokos() {
  for (var _index2 = 0; _index2 < hinokos.length; _index2++) {
    var hinokoObj = hinokos[_index2];

    if (hinokoObj.aliveClock >= deadClock) {
      hinokos.splice(_index2, 1);
      _index2 -= 1;
